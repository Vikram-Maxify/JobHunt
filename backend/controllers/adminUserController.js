        ...
        const stats = await User.aggregate([
            {
                $group: {
                    ...
                    recruiterUsers: { $sum: { $cond: [{ $eq: ['$role', 'recruiter'] }, 1, 0] } },
                    jobSeekerUsers: { $sum: { $cond: [{ $eq: ['$role', 'jobseeker'] }, 1, 0] } }
                }
            }
        ]);
...
exports.updateUserAdmin = async (req, res) => {
    try {
        ...
        if (currentCompany !== undefined) user.currentCompany = String(currentCompany).trim();
        ...
        if (preferredJobRole !== undefined) user.preferredJobRole = String(preferredJobRole).trim();
        if (preferredLocation !== undefined) user.preferredLocation = String(preferredLocation).trim();
        if (employmentType !== undefined) user.employmentType = String(employmentType).trim();
        if (salaryExpectation !== undefined) user.salaryExpectation = String(salaryExpectation).trim();
